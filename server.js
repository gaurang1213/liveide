...
const ot = require('./lib/ot/otText');
...
app.prepare().then(() => {
  ...
  const roomFiles = {};
  // roomId -> { revs: { [fileId]: number }, history: { [fileId]: Array<op> } }
  const roomMeta = {};
  ...
  function handleWebSocketMessage(socket, decodedMessage) {
    try {
      ...
      if (action === RAW_ACTIONS.JOIN && senderRoomId) {
        ...
        if (!roomMeta[senderRoomId]) {
          roomMeta[senderRoomId] = { revs: {}, history: {} };
        }
      ...
      // --- OT op handling ---
      if (action === RAW_ACTIONS.OP && senderRoomId && fileId && Array.isArray(rest.ops)) {
        if (!roomFiles[senderRoomId]) roomFiles[senderRoomId] = {};
        if (!roomMeta[senderRoomId]) roomMeta[senderRoomId] = { revs: {}, history: {} };
        const meta = roomMeta[senderRoomId];
        const history = (meta.history[fileId] ||= []);
        const currentRev = meta.revs[fileId] ||= 0;
        const baseRev = typeof rest.baseRev === 'number' ? rest.baseRev : 0;
        const ops = rest.ops;

        let opPrime = ops;
        if (baseRev < currentRev) {
          for (let i = baseRev; i < currentRev; i++) {
            const histOp = history[i];
            if (histOp) {
              opPrime = ot.transform(opPrime, histOp);
            }
          }
        }

        const prev = roomFiles[senderRoomId][fileId] || '';
        const nextContent = ot.apply(prev, opPrime);
        roomFiles[senderRoomId][fileId] = nextContent;
        history[currentRev] = opPrime;
        meta.revs[fileId] = currentRev + 1;
        saveRoomSnapshot(senderRoomId);

        const ack = JSON.stringify({
          action: RAW_ACTIONS.OP_ACK,
          payload: { fileId, rev: meta.revs[fileId], ts: (payload && payload.ts) || Date.now() },
        });
        sendMessage(ack, socket);

        const ob = JSON.stringify({
          action: RAW_ACTIONS.OP_BROADCAST,
          payload: { fileId, ops: opPrime, rev: meta.revs[fileId], ts: (payload && payload.ts) || Date.now() },
        });
        broadcastMessage(senderRoomId, ob, socket._socketId);
        return;
      }
