...
import * as ot from "@/lib/ot/otText";
...
export function useCollaboration({ playgroundId }: UseCollaborationOptions) {
  ...
  const pendingRef = useRef<Record<string, Array<{ ops: any[]; baseRev: number }>>>({});
  const otEnabledRef = useRef<boolean>(false);
      ...
      try {
        ...
        if (action === "joined") {
          ...
          Object.keys(files).forEach((fid) => {
            docsRef.current[fid] = files[fid] as string;
            if (revsRef.current[fid] == null) revsRef.current[fid] = 0;
          ...
          const { fileId, rev } = payload || {};
          if (fileId && typeof rev === 'number') {
          ...
          if (fileId && Array.isArray(ops)) {
            let incoming = ops as any[];
            const pend = pendingRef.current[fileId] || [];
            for (const p of pend) {
              incoming = ot.transform(incoming, p.ops);
            }
